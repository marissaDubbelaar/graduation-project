$(document).ready(function () {
	...
	$("body").on("click", "#submitQEbutton", function(){
		...
		var attrNames = [];
		var availableGenes = [];
		// Creating a hidden table to download the informative content.
		var hiddenTableContent = '<table id=tpmTable" class="table table-striped table-hover table-condensed table-responsive header-fixed"><thead><tr>';
		// Creating a table that is shown on the website.
		var tableContent = '<table id=tpmTable" class="table table-striped table-hover table-condensed table-responsive header-fixed"><tbody id="tpmContent" class="searchable">';
		$.get("/api/v2/TPM_"+GEODOnPage[0].replace(/-/g,'')).done(function(data){
			...
			$.each(attr, function(t, types){
				...
				if(!(attr[t]["name"].endsWith("low") || attr[t]["name"].endsWith("high"))){
					...
					if (attr[t]["name"].length < 4){
						...
						hiddenTableContent += '<th>'+ attr[t]["name"].replace(/_/g, " ").toUpperCase()+'</th>';	
					} else {
						// The rest of the cell type names are capitalized.
						hiddenTableContent += '<th>'+ capitalizeEachWord(attr[t]["name"].replace(/_/g, " "))+'</th>';
					}
				}
			 });
			// The rest of the header of the hidden table is added together with the body.
			hiddenTableContent += '</tr></thead><tbody id="hiddenTpmContent" class="searchable"></tbody></table>'
			// This information is added into the div with the id "hiddenQETable"
			$("#hiddenQETable").append(hiddenTableContent);
			...
			$.each(data, function(i, content){
				...
				// The genes are added onto the table that is shown on the website.
				tableContent += '<tr class="TpmVals" id="'+ content["external_gene_name"] +'"><td><b>' + content["external_gene_name"] + '</b></td></tr>'
				...
				// For each cell type name (this part is for the hidden div)
				$.each(attrNames, function(n, names){
					if (n === 0) {
						...
						// Information is added onto the div with id "hiddenTpmContent".
						$('#hiddenTpmContent').append(stringToAppend);
			...
			// Shown div content is added onto the div with id "QETable".
		...
		$.each(data, function(i, content){
			$.each(attr, function(t, types){
				...
				if (t === 0) {
					...
					// Information on position 2 when performing t % 3 are saved as the low TPM values.
					dict["Low TPM"] = data[i][attr[t]["name"]]
				} else if (t % 3 === 0 && t !== 0) {
					// Information on position 0 when performing t % 3 are saved as the high TPM values.
					dict["High TPM"] = data[i][attr[t]["name"]]
					console.log(dict)
